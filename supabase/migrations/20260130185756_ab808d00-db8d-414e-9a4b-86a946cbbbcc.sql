-- Drop existing function and recreate with simplified logic
DROP FUNCTION IF EXISTS public.get_all_hubla_transactions(timestamptz, timestamptz, text, integer);
...
BEGIN
  ...
  WHERE ht.sale_status IN ('completed', 'refunded')
    AND ht.source IN ('hubla', 'manual')  -- Only hubla and manual sources
