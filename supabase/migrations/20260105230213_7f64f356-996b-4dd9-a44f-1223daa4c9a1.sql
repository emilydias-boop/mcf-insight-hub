...
-- Modificar get_sdr_metrics_v2 para contar reagendamentos após QUALQUER saída do funil
-- (não apenas após No-Show), aproximando a contagem do Clint
...
BEGIN
  WITH 
  -- Pegar todos os eventos de stage_change (histórico completo para análise)
  all_events AS (
    SELECT 
      da.deal_id,
      da.created_at,
  ...
  -- Identificar TODOS os agendamentos de cada deal no período
  all_agendamentos_no_periodo AS (
    SELECT 
      da.deal_id,
      da.owner_email,
      da.created_at as agendamento_at,
      ROW_NUMBER() OVER (PARTITION BY da.deal_id ORDER BY da.created_at) as agendamento_order
  ...
  -- Reagendamento = qualquer agendamento após o primeiro (não apenas após no-show)
  agendamentos_classificados AS (
    SELECT 
      ...
      ap.agendamento_at,
      CASE 
        WHEN fa.primeiro_agendamento_at = ap.agendamento_at THEN 'primeiro'
        ELSE 'reagendamento'
      END as tipo
    FROM all_agendamentos_no_periodo ap
    LEFT JOIN first_agendamento fa ON fa.deal_id = ap.deal_id
  ),
  
  -- No-shows (owner no momento) - apenas primeiro no-show por deal
  no_shows AS (
    SELECT 
      da.deal_id,
  ...
  -- Realizadas (por intermediação do primeiro agendamento)
  realizadas AS (
    SELECT 
      da.deal_id,
  ...
  -- Contratos (por intermediação do primeiro agendamento)
  contratos AS (
    SELECT 
      da.deal_id,
  ...
  sdr_metrics AS (
    ...
    FROM (
      -- 1º Agendamentos (deal nunca teve agendamento antes)
      SELECT 
        ac.owner_email as sdr_email,
        ac.deal_id::text as primeiro_agendamento_deal,
      ...
      WHERE ac.tipo = 'primeiro'
        AND (sdr_email_filter IS NULL OR LOWER(ac.owner_email) = LOWER(sdr_email_filter))
      
      UNION ALL
      
      -- Reagendamentos (qualquer agendamento após o primeiro)
      SELECT 
        ac.owner_email as sdr_email,
        NULL::text as primeiro_agendamento_deal,
        ac.deal_id::text || '_' || ac.agendamento_at::text as reagendamento_deal, -- Contar cada reagendamento
        NULL::text as no_show_deal,
        NULL::text as realizada_deal,
        NULL::text as contrato_deal
      FROM agendamentos_classificados ac
      WHERE ac.tipo = 'reagendamento'
        AND (sdr_email_filter IS NULL OR LOWER(ac.owner_email) = LOWER(sdr_email_filter))
      
      UNION ALL
      
      -- No-Shows (owner no momento do no-show)
      SELECT 
