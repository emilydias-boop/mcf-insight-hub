...
-- Corrigir a função detect_ghost_appointments para extrair owner_email corretamente
-- O campo está em metadata->'deal'->>'user', não em metadata->>'deal_user'
...
BEGIN
  ...
  all_events AS (
    SELECT 
      ...
      da.to_stage,
      da.from_stage,
      -- CORREÇÃO: buscar owner_email em múltiplos campos possíveis
      COALESCE(
        da.metadata->>'owner_email',
        da.metadata->'deal'->>'user',
