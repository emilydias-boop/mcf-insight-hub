-- Nova RPC que retorna TODOS os movimentos para R1 Agendada com indicação se conta ou não
...
BEGIN
  ...
  -- Todos os movimentos para R1 Agendada no período
  all_movements AS (
    SELECT 
      ea.deal_id,
      ...
      CASE 
        -- 1º Agendamento: é o primeiro movimento para R1 Agendada do deal
        WHEN fa.primeiro_agendamento_at = ea.created_at THEN '1º Agendamento'
        -- Reagendamento após No-Show: existe no-show antes deste agendamento
        WHEN EXISTS (
        ...
        -- Qualquer outro reagendamento não conta
  ...
  enriched_movements AS (
    SELECT 
      ...
      am.data_agendamento,
      am.agendador_email as intermediador,
      cs.status_atual,
      cs.current_owner,
      CASE WHEN LOWER(cs.current_owner) != LOWER(am.agendador_email) 
           THEN cs.current_owner ELSE NULL END as closer,
      o.name as origin_name,
      d.probability,
      am.tipo_contagem as tipo,
      am.conta,
      am.from_stage
    FROM all_movements am
    INNER JOIN crm_deals d ON d.id = am.deal_id::uuid
    LEFT JOIN crm_contacts c ON c.id = d.contact_id
    LEFT JOIN current_status cs ON cs.deal_id = am.deal_id
    LEFT JOIN crm_origins o ON o.id = d.origin_id
    WHERE (sdr_email_filter IS NULL OR LOWER(am.agendador_email) = LOWER(sdr_email_filter))
  )
