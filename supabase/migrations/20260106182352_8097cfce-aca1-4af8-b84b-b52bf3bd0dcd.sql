...
-- Primeiro dropar a função existente que tem tipo de retorno diferente
DROP FUNCTION IF EXISTS public.get_sdr_all_movements_v2(date, date, text);

-- Recriar função get_sdr_all_movements_v2 incluindo "Novo Lead" como from_stage válido
  ...
  WITH r1_movements AS (
    ...
    WHERE da.activity_type = 'stage_change'
      ...
      AND (
        ...
        OR da.from_stage ILIKE '%Novo Lead%'
      )
  ),
  movement_counts AS (
    SELECT 
...
-- Atualizar função get_sdr_metrics_v2 para incluir "Novo Lead" como from_stage válido
