-- Corrigir função get_sdr_metrics_v2 para contar corretamente os agendamentos
-- Problema: ROW_NUMBER era aplicado antes do filtro, excluindo movimentos válidos
...
BEGIN
  WITH r1_movements AS (
    -- Primeiro filtra APENAS movimentos válidos para R1 Agendada com origem válida
    SELECT 
      da.deal_id,
      da.created_at,
    ...
    WHERE da.activity_type IN ('stage_change', 'stage_changed')
      ...
      -- Filtrar para R1 Agendada ANTES de qualquer processamento
      AND (
  ...
  filtered_movements AS (
    -- Filtrar origem válida e classificar tipo
    SELECT 
      rm.deal_id,
      rm.created_at,
      rm.mov_date,
  ...
  deduplicated AS (
    -- AGORA aplica ROW_NUMBER apenas nos movimentos já filtrados
    -- Um deal conta apenas uma vez no período (primeiro movimento válido)
