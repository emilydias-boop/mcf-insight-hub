-- Funil por Canal alinhado ao Painel Comercial
-- Replica get_sdr_metrics_from_agenda, mas agrupa por canal de aquisição em vez de SDR.
...
BEGIN
  WITH deal_channels AS (
    -- Classifica cada deal em A010 / ANAMNESE / OUTROS
    SELECT
      d.id AS deal_id,
      CASE
        -- A010: tag, nome de origem ou lead_channel contém A010
        WHEN UPPER(COALESCE(o.name,'')) LIKE '%A010%'
          OR UPPER(COALESCE(d.custom_fields->>'lead_channel','')) LIKE '%A010%'
          OR EXISTS (
            SELECT 1 FROM jsonb_array_elements_text(COALESCE(d.tags, '[]'::jsonb)) t(val)
            WHERE UPPER(t.val) LIKE '%A010%'
          )
        THEN 'A010'
        -- ANAMNESE: junta LIVE + ANAMNESE + ANAMNESE-INSTA + LANÇAMENTO
  ...
  -- R1 Agendada: até 2 dias distintos por deal contam
  dedup_agendada AS (
  ...
  -- Entradas: deals criados no período, dentro da BU (via origin_id)
  bu_origins AS (
    SELECT DISTINCT origin_id
    FROM crm_deals d
    WHERE bu_filter IS NULL
       OR EXISTS (
         SELECT 1 FROM bu_pipeline_map bpm
         WHERE bpm.origin_id = d.origin_id AND bpm.bu = bu_filter
       )
  ),
  entradas_cte AS (
...
GRANT EXECUTE ON FUNCTION public.get_channel_funnel_metrics(text, text, text) TO authenticated, anon;