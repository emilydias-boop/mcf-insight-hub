-- Atualiza RPC get_carrinho_r2_attendees para suportar corte por DATA DO CONTRATO PAGO
-- Adiciona p_apply_contract_cutoff (default false): quando true, leads são considerados
-- "dentro do corte" se contract_paid_at estiver na janela [p_window_start, p_window_end).
-- Retorna campo extra `dentro_corte` para o frontend filtrar Aprovados tardios.
...
WITH raw_r2 AS (
  ...
  WHERE ms.meeting_type = 'r2'
    ...
    AND (
      ...
      OR (
        -- Quando aplicando corte por contrato, também incluir leads cujo
        -- contract_paid_at está dentro da janela (mesmo que a R2 tenha sido
        -- agendada antes/depois). Isso captura R2s antigas que recém aprovaram
        -- e contratos pagos pós-Quarta antes do corte de Sex 12:00.
        p_apply_contract_cutoff = true
        AND msa.contract_paid_at >= p_window_start
        AND msa.contract_paid_at < p_window_end
        AND msa.carrinho_week_start IS NULL
      )
    )
),
...
SELECT
  ...
  -- dentro_corte: quando NÃO aplicando corte → todos true (compatibilidade).
  -- Quando aplicando corte → true apenas se o contract_paid_at do attendee R2
  -- estiver dentro da janela [p_window_start, p_window_end).
  -- Encaixados manualmente sempre contam como dentro do corte (intervenção humana).
  CASE
    WHEN p_apply_contract_cutoff = false THEN true
    WHEN d.is_encaixado = true THEN true
    WHEN d.contract_paid_at IS NOT NULL
         AND d.contract_paid_at >= p_window_start
         AND d.contract_paid_at < p_window_end THEN true
    -- Carry-over: contrato pago ANTES da janela, R2 dentro da janela
    -- (lead antigo que finalmente foi aprovado nesta safra)
    WHEN d.contract_paid_at IS NOT NULL
         AND d.contract_paid_at < p_window_start
