-- Corrigir No-Show para usar contagem por status em vez de cálculo matemático
-- Isso garante que apenas reuniões marcadas como 'no_show' sejam contadas
...
BEGIN
  WITH sdr_metrics AS (
    SELECT 
      ...
      -- Agendamentos: criados no período (usa COALESCE para fallback)
      COUNT(DISTINCT CASE 
      ...
      -- CORREÇÃO: No-shows por STATUS (não mais por cálculo)
      COUNT(DISTINCT CASE 
        WHEN (ms.scheduled_at AT TIME ZONE 'America/Sao_Paulo')::date >= start_date::DATE 
         AND (ms.scheduled_at AT TIME ZONE 'America/Sao_Paulo')::date <= end_date::DATE 
