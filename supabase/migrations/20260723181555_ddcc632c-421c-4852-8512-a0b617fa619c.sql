...
-- 1) Dedup existente: para cada grupo (customer_email, product_code, minuto de sale_date), manter o que tem mais parcelas (empate: mais antigo)
WITH grupos AS (
  SELECT t.id,
         coalesce(lower(t.customer_email), t.customer_phone, t.customer_name) AS chave_cliente,
         t.product_code,
         date_trunc('hour', t.sale_date) AS bucket,
         (SELECT count(*) FROM ar_parcelas p WHERE p.titulo_id=t.id) AS n_parc,
         t.created_at
  FROM ar_titulos t
  WHERE t.hubla_transaction_id IS NOT NULL
),
ranked AS (
  SELECT id, chave_cliente, product_code, bucket,
         row_number() OVER (
           PARTITION BY chave_cliente, product_code, bucket
           ORDER BY n_parc DESC, created_at ASC
         ) rn
  FROM grupos
),
to_delete AS (
...
DELETE FROM ar_titulos WHERE id IN (SELECT id FROM to_delete);

-- 2) Atualizar trigger para deduplicar também por cliente+produto+janela de 1h
...
BEGIN
  ...
  IF v_code IS NULL OR NOT v_code = ANY(ARRAY['A001','A002','A003','A004','A005','A006','A007','A008','A009']) THEN
    RETURN NEW;
  END IF;
  ...
  -- NOVO: bloqueia duplicidade por mesmo cliente+produto em janela de 1h
  -- (ex.: Hubla dispara NewSale e invoice.payment_succeeded para a mesma venda)
