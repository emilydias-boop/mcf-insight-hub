...
BEGIN
  WITH raw_attendees AS (
    ...
    WHERE msa.status != 'cancelled'
      ...
      -- ⚠️ NÃO filtrar por sdr_email_filter aqui — precisamos de TODOS os attendees
      -- do squad para que o ranking global por deal_id seja consistente entre
      -- "Minhas Reuniões" (filtra 1 SDR) e "Painel da Equipe" (todos os SDRs).
      AND (
        bu_filter IS NULL
  ...
  -- ✅ Aplica o filtro de SDR APÓS o ranking, preservando ordem global
