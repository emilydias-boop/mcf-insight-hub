-- Fix deduplication tie-breaker: prioritize hubla/manual over make when dates are equal
-- This ensures official transactions are marked as "first" instead of make tracking records
...
BEGIN
  ...
  ranked_transactions AS (
    SELECT
      ...
      ROW_NUMBER() OVER (
        PARTITION BY
          ...
          END
        -- Fix: Add tie-breaker to prioritize hubla > manual > make
    ...
    WHERE
      ...
      -- Excluir registros auxiliares do Make (tracking)
      AND NOT (
        ht.source = 'make'
