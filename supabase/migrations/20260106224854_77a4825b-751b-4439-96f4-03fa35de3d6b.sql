...
BEGIN
  WITH r1_movements AS (
    -- Filtrar APENAS movimentos para R1 Agendada já aqui
    SELECT 
      da.deal_id,
      da.created_at,
    ...
    WHERE da.activity_type IN ('stage_change', 'stage_changed')
      ...
      -- Filtrar para R1 Agendada ANTES do ROW_NUMBER
      AND (
  ...
  deduplicated AS (
    -- Primeiro agendamento válido por deal no período
    SELECT * FROM filtered_movements WHERE rn = 1
  ),
  noshow_movements AS (
    SELECT DISTINCT
      da.deal_id,
