-- Otimização: Remove subconsulta correlacionada de deal_tags que causava timeout
-- A coluna deal_tags não é utilizada no frontend (SalesReportPanel.tsx exporta '' sempre)
-- Impacto esperado: tempo de execução de >30s (timeout) para <1s
...
BEGIN
  ...
  SELECT 
    ...
    ht.source::text,
    ht.gross_override,
    ARRAY[]::text[] as deal_tags  -- Retorna array vazio (otimização: remove subconsulta N+1)
