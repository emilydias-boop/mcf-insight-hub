DROP FUNCTION IF EXISTS public.caucoes_efetivas(date, date, text);

CREATE OR REPLACE FUNCTION public.caucoes_efetivas(p_from date, p_to date, p_bu text DEFAULT 'incorporador'::text)
 RETURNS TABLE(attendee_id uuid, deal_id uuid, lead_name text, eff_date date, fonte text, contract_paid_at timestamp with time zone, refunded_at timestamp with time zone, closer_id uuid, closer_name text, closer_bu text, sdr_id uuid, sdr_email text, sdr_name text, segment text, valor numeric, origin_id uuid)
 LANGUAGE sql
