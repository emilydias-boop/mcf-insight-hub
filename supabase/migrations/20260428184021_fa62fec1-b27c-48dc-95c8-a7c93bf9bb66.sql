...
AS $function$
  ...
  WHERE s.role_type = 'sdr'
    ...
    -- Exclui usuários cuja role real em user_roles seja administrativa ou de closer
    AND NOT EXISTS (
      SELECT 1 FROM public.user_roles ur
      WHERE ur.user_id = s.user_id
        AND ur.role IN ('admin','manager','coordenador','assistente_administrativo','closer','closer_sombra')
    );
$function$;
