-- Atualiza RPC get_carrinho_r2_attendees para priorizar r2_status_name = 'Aprovado' sobre 'Próxima Semana'
-- na deduplicação por telefone, resolvendo casos onde o mesmo lead tem múltiplos R2 na semana
...
with_phone_key AS (
  SELECT r.*,
    ...
    CASE lower(COALESCE(r.attendee_status, ''))
      ...
      ELSE 50
    END AS status_score,
    -- Score por r2_status_name: "Aprovado" deve vencer "Próxima Semana", "Reembolso", etc
  ...
  SELECT *,
    ROW_NUMBER() OVER (
      ...
      ORDER BY
        ...
        r2_status_score DESC,     -- 2º: priorizar Aprovado/Compareceu sobre Próxima Semana/Reembolso
        status_score DESC,        -- 3º: status do attendee (contract_paid > scheduled > refunded)
        scheduled_at DESC         -- 4º: tiebreaker = mais recente
    ) AS rn
  FROM with_phone_key
),
deduped AS (
