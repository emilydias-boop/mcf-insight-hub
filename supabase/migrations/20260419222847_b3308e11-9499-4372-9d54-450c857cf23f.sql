...
CREATE FUNCTION public.get_carrinho_r2_attendees(
  p_week_start date,
...
RETURNS TABLE(
  ...
  carrinho_updated_at timestamp with time zone,
  ...
  contact_name text,
  partner_name text,
  contract_paid_at timestamp with time zone,
  ...
  deal_name text,
  r1_scheduled_at timestamp with time zone,
  r1_closer_name text,
  r1_closer_id uuid,
  ...
  effective_contract_date timestamp with time zone,
...
SELECT
  ...
  CASE
    ...
    WHEN COALESCE(w.r1_contract_paid_at, w.contract_paid_at, w.hubla_contract_at) IS NOT NULL
         AND COALESCE(w.r1_contract_paid_at, w.contract_paid_at, w.hubla_contract_at) >= p_window_start
         AND COALESCE(w.r1_contract_paid_at, w.contract_paid_at, w.hubla_contract_at) < p_window_end THEN true
    WHEN COALESCE(w.r1_contract_paid_at, w.contract_paid_at, w.hubla_contract_at) IS NOT NULL
         AND COALESCE(w.r1_contract_paid_at, w.contract_paid_at, w.hubla_contract_at) < p_window_start
         AND w.scheduled_at >= p_window_start
         AND w.scheduled_at < p_window_end
         AND (w.parceria_first_at IS NULL OR w.parceria_first_at >= p_window_start) THEN true
    ELSE false
  END AS dentro_corte,
