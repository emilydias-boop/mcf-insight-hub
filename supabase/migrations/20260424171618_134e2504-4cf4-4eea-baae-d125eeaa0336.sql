...
BEGIN
  ...
  deal_signals AS (
    SELECT
      ...
      -- Sinal A010 (apenas por TAG do deal — origem/pipeline NÃO classifica mais)
      EXISTS (
        SELECT 1 FROM unnest(COALESCE(d.tags, ARRAY[]::text[])) AS t(val)
        WHERE UPPER(t.val) LIKE '%A010%'
      ) AS has_a010_tag,
      -- Sinal ANAMNESE (apenas por TAG do deal, ignorando tags com INCOMPLET)
      EXISTS (
        SELECT 1 FROM unnest(COALESCE(d.tags, ARRAY[]::text[])) AS t(val)
  ...
  deal_channels AS (
    SELECT
      ...
      ds.origin_id,
      CASE
        -- 1. Compra A010 recente (≤30 dias antes ou ≤1 dia depois do deal) → A010
        ...
        THEN 'A010'
        -- 2. Tag válida ANAMNESE/LIVE/LANÇAMENTO (sem INCOMPLET) → ANAMNESE
