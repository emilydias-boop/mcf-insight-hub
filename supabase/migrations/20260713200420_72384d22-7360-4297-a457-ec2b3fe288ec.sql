...
-- Função auxiliar: obtém preço de referência do produto
CREATE OR REPLACE FUNCTION public.ar_get_reference_price(p_code text)
RETURNS numeric
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
DECLARE
  v numeric;
BEGIN
  SELECT reference_price INTO v
  FROM public.product_configurations
  WHERE product_code = p_code AND is_active = true AND reference_price IS NOT NULL
  ...
  -- Fallback hardcoded
  RETURN CASE p_code
    WHEN 'A001' THEN 12500
    WHEN 'A002' THEN 7500
    WHEN 'A003' THEN 7503
    WHEN 'A004' THEN 5503
    WHEN 'A009' THEN 17500
    ELSE 0
  END;
END; $$;

-- Nova versão do trigger com suporte a MCF PAY
...
BEGIN
  ...
  IF coalesce(NEW.source,'') = 'mcfpay' THEN
    ...
    -- Venda à vista (parcela cobre o preço integral)
    ...
    -- Venda parcelada MCF PAY: buscar título aberto do mesmo cliente/produto (últimos 12 meses)
    ...
    IF v_existing_titulo.id IS NOT NULL THEN
      -- Tenta baixar próxima parcela pendente
      ...
      ELSE
        -- Não há pendente: cria uma nova parcela paga adicional
      ...
      -- Se todas as parcelas estão pagas, quita o título
    ...
    -- Não há título existente: cria título parcelado, parcela 1 paga + demais pendentes até integralizar
    IF v_ref <= 0 THEN
    ...
    -- Demais parcelas pendentes de igual valor
