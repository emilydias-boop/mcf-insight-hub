...
BEGIN
  ...
  -- ============================================================
  -- KPI: R1 AGENDADA (planejamento — janela completa, inclui futuro)
  -- ============================================================
  dedup_agendada AS (
  ...
  -- ============================================================
  -- KPI: R1 REALIZADA (fato — cap em hoje)
  -- ============================================================
  dedup_realizada AS (
  ...
  -- ============================================================
  -- KPI: NO-SHOWS (fato — cap em hoje + cap por lead 1/2)
  -- ============================================================
  noshow_per_lead AS (
    SELECT sdr_email, deal_id,
      LEAST(
  ...
  -- ============================================================
  -- KPI: SEM STATUS (híbrido)
  -- - is_future_window=true → pendentes vivos (scheduled_at <= NOW())
  -- - is_future_window=false → backlog histórico (range completo)
  -- ============================================================
  ...
  -- ============================================================
  -- KPI: AGENDAMENTOS (fato — quando foi criado, cap em hoje)
  -- ============================================================
  agendamentos_cte AS (
    SELECT sdr_email, COUNT(*) as agendamentos
    FROM filtered_ranked
    WHERE ordem <= 2
      AND (effective_booked_at AT TIME ZONE 'America/Sao_Paulo')::date BETWEEN start_d AND effective_end
    GROUP BY sdr_email
  ),
  -- ============================================================
  -- KPI: CONTRATOS (fato — cap em hoje)
  -- ============================================================
  contratos_cte AS (
  ...
  -- ============================================================
  -- Universo de SDRs com qualquer atividade no range
  -- ============================================================
  sdr_universe AS (
