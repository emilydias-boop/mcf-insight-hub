...
BEGIN
  ...
  ranked_movements AS (
    SELECT
      sdr_email, sdr_name, deal_id, effective_booked_at, meeting_day, status, contract_paid_at,
  ...
  -- Cap 2 sempre (sem data de corte) — alinhado com Consórcio
  noshow_per_lead AS (
    SELECT sdr_email, deal_id,
      LEAST(COUNT(DISTINCT meeting_day), 2) as noshow_count
  ...
  -- Sem status: invited/rescheduled/sem_sucesso/recurrence_recognized (cap 2 por lead)
  sem_status_per_lead AS (
    SELECT sdr_email, deal_id,
      LEAST(COUNT(DISTINCT meeting_day), 2) as sem_status_count
    FROM filtered_attendees
    WHERE status IN ('invited','rescheduled','sem_sucesso','recurrence_recognized')
      AND meeting_day BETWEEN start_date::DATE AND end_date::DATE
    GROUP BY sdr_email, deal_id
  ),
  agendamentos_cte AS (
  ...
  contratos_cte AS (
    ...
    FROM filtered_attendees
    WHERE (contract_paid_at AT TIME ZONE 'America/Sao_Paulo')::date BETWEEN start_date::DATE AND end_date::DATE
  ...
  sdr_universe AS (
    ...
    WHERE meeting_day BETWEEN start_date::DATE AND end_date::DATE
      ...
      OR (contract_paid_at AT TIME ZONE 'America/Sao_Paulo')::date BETWEEN start_date::DATE AND end_date::DATE
  ),
  ...
  sem_status_agg AS (
    SELECT sdr_email, SUM(sem_status_count)::int as sem_status
    FROM sem_status_per_lead
    GROUP BY sdr_email
  ),
  dedup_agg AS (
    SELECT sdr_email,
  ...
  sdr_stats AS (
    SELECT u.sdr_email, u.sdr_name,
      ...
      COALESCE(ss.sem_status, 0) as sem_status,
