...
as $$
declare
  v_comissao numeric := 74.55; -- 15% de R$ 497 (unidade de comissao MCF Pay)
  v_result jsonb;
begin
  with vendas as (
    select t.id as ref_id, t.linked_deal_id as deal_id, 'venda'::text as kind
  ...
  reembolsos as (
    select distinct on (a.deal_id) a.id::text as ref_id, a.deal_id, 'reembolso'::text as kind
