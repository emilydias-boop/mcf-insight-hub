...
BEGIN
  ...
  WHERE ht.sale_status = 'completed'
    ...
    -- Converte sale_date para timezone de Brasília antes de comparar
    AND (ht.sale_date AT TIME ZONE 'America/Sao_Paulo')::date >= COALESCE(p_start_date::date, '2020-01-01'::date)
    AND (ht.sale_date AT TIME ZONE 'America/Sao_Paulo')::date <= COALESCE(p_end_date::date, CURRENT_DATE)
    AND (
      p_search IS NULL 
