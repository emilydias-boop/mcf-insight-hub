-- Atualizar RPC get_sdr_metrics_v2 para contar no-shows corretamente (1x por deal)
...
BEGIN
  ...
  -- NOVO: Conta movimentos para No-Show no período (1x por deal por SDR)
  noshow_movements AS (
    SELECT DISTINCT
      da.deal_id,
      COALESCE(da.metadata->>'owner_email', da.metadata->>'deal_user') as sdr_email
  ...
  sdr_metrics AS (
    SELECT 
      ...
      -- CORRIGIDO: conta deals distintos que foram para No-Show no período
      (SELECT COUNT(DISTINCT nm.deal_id) 
       FROM noshow_movements nm 
       WHERE LOWER(nm.sdr_email) = LOWER(rm.sdr_email)) as no_shows,
  ...
  -- Totais de no-shows para o summary
  noshow_totals AS (
    SELECT 
  ...
  SELECT jsonb_build_object(
    ...
    'summary', jsonb_build_object(
      ...
      'total_no_shows', COALESCE((SELECT SUM(no_show_count) FROM noshow_totals), 0),
