...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
      ...
      'venda_produto', coalesce(sale.produtos, d.product_name),
      'venda_valor', coalesce(sale.valor, CASE WHEN coalesce(st.stage_name,'') ILIKE 'Venda%realizada' THEN nullif(d.value,0) END),
  ...
  vendas AS (
    SELECT
      count(*) FILTER (WHERE (dl.stage_moved_at AT TIME ZONE 'America/Sao_Paulo')::date = v_today) AS qtd_dia,
      coalesce(sum(coalesce(dl.value,0)) FILTER (WHERE (dl.stage_moved_at AT TIME ZONE 'America/Sao_Paulo')::date = v_today),0) AS valor_dia,
      count(*) FILTER (WHERE (dl.stage_moved_at AT TIME ZONE 'America/Sao_Paulo')::date >= v_week_start) AS qtd_semana,
      coalesce(sum(coalesce(dl.value,0)) FILTER (WHERE (dl.stage_moved_at AT TIME ZONE 'America/Sao_Paulo')::date >= v_week_start),0) AS valor_semana
    FROM crm_deals dl
    ...
    WHERE dl.origin_id = ANY(v_origins)
      ...
      AND dl.stage_moved_at >= (v_week_start - interval '1 day')
  )
  SELECT jsonb_build_object(
    ...
    'vendas_realizadas', jsonb_build_object(
      'criterio', 'negocios movidos para stage Venda realizada',
