-- Fix type mismatch: deal_activities.deal_id (text) vs crm_deals.id (uuid)
...
BEGIN
  ...
  primeiro_agendamento AS (
    ...
    JOIN deal_data dd ON da.deal_id = dd.deal_id::text
  ...
  -- Contratos via Hubla transactions crossed with CRM
  contratos_hubla AS (
    SELECT DISTINCT 
      ht.customer_email,
