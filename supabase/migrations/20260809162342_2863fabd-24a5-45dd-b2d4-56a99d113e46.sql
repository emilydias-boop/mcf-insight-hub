CREATE TABLE public.hubla_transaction_autolink_log (
...
GRANT SELECT ON public.hubla_transaction_autolink_log TO authenticated;
GRANT ALL ON public.hubla_transaction_autolink_log TO service_role;

ALTER TABLE public.hubla_transaction_autolink_log ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Admins can view autolink log"
ON public.hubla_transaction_autolink_log
FOR SELECT TO authenticated
USING (public.has_role(auth.uid(), 'admin'));

CREATE INDEX idx_autolink_log_tx ON public.hubla_transaction_autolink_log(transacao_id);
...
BEGIN
  FOR v_tx IN
    ...
    LIMIT p_limit
  LOOP
    v_deal := NULL; v_crit := NULL; v_att := NULL;
    ...
    -- Candidatos por telefone (só se e-mail não achou)
    ...
    ELSE
      -- Desempate: pipeline Incorporador + reunião mais próxima da sale_date
      WITH inc AS (
        SELECT t.deal_id
        FROM tmp_cand t
    ...
    -- Attendee de R1/R2 não cancelado, mais recente
...
REVOKE ALL ON FUNCTION public.autolink_orphan_a000_transactions(integer) FROM anon, authenticated;
GRANT EXECUTE ON FUNCTION public.autolink_orphan_a000_transactions(integer) TO service_role;