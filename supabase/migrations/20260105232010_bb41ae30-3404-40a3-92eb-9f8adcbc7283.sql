-- Drop and recreate get_sdr_metrics_v2 with correct webhook_events structure
...
BEGIN
  ...
  webhook_r1_events AS (
    SELECT 
      ...
      we.created_at,
      ROW_NUMBER() OVER (
        PARTITION BY we.event_data->>'deal_id', DATE(we.created_at AT TIME ZONE 'America/Sao_Paulo')
    ...
    WHERE we.event_type = 'deal.stage_changed'
      ...
      AND (sdr_email_filter IS NULL OR lower(we.event_data->>'deal_user') = lower(sdr_email_filter))
  ),
  ...
  -- Passo 5: No-shows (movimentos para No-Show)
  sdr_no_shows AS (
    SELECT 
    ...
    GROUP BY lower(d.owner_id)
  ),
  
  -- Passo 6: Realizadas (movimentos para Reunião 01 Realizada)
  sdr_realizadas AS (
    SELECT 
      lower(d.owner_id) as sdr_email,
  ...
  -- Passo 7: Contratos (movimentos para Contrato Pago ou Venda realizada)
  sdr_contratos AS (
    SELECT 
      lower(d.owner_id) as sdr_email,
    ...
    JOIN crm_deals d ON da.deal_id = d.id::text
    WHERE da.activity_type = 'stage_change'
      AND da.to_stage IN ('Contrato Pago', 'Venda realizada')
      AND DATE(da.created_at AT TIME ZONE 'America/Sao_Paulo') BETWEEN start_date AND end_date
      AND (sdr_email_filter IS NULL OR lower(d.owner_id) = lower(sdr_email_filter))
    GROUP BY lower(d.owner_id)
  ),
