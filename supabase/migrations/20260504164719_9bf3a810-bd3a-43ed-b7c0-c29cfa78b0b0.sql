...
BEGIN
  ...
  WITH base AS (
    SELECT
      ...
      d.name as d_name, d.probability,
  ...
  -- 1 linha por (sdr, deal, day) com flags
  by_day AS (
    SELECT booker_email as sdr_email, deal_id, meeting_day,
      BOOL_OR(status IN ('completed','contract_paid','refunded')) as has_real,
      BOOL_OR(status='no_show') as has_ns,
      -- escolhe um attendee representativo (preferindo no-show > pendente)
    ...
    FROM base
    GROUP BY booker_email, deal_id, meeting_day
  ),
  -- Cap 2 dias por deal (igual R1 Agendada)
  ranked AS (
    SELECT *, ROW_NUMBER() OVER (PARTITION BY sdr_email, deal_id ORDER BY meeting_day) as day_ord,
      ROW_NUMBER() OVER (PARTITION BY sdr_email, deal_id, has_ns ORDER BY meeting_day) as ns_ord
    FROM by_day
  ),
  -- Pendentes = dias dentro do cap 2 que NÃO viraram realizada e que também
  -- NÃO foram contabilizados no cap de no-show (1 antes de 2026-05-01, 2 depois).
  pendentes AS (
    SELECT *,
      CASE
