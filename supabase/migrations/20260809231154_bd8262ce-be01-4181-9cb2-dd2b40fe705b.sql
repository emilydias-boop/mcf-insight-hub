-- LAST TOUCH na atribuição de SDR da venda (agg=daily)
...
BEGIN
  ...
  vd_base AS (
    SELECT (dl.stage_moved_at AT TIME ZONE 'America/Sao_Paulo')::date AS dia,
           ...
           -- LAST TOUCH: SDR do ÚLTIMO agendamento de R1 antes da venda
           coalesce((
             SELECT p2.full_name
  ...
  vd_closer_r1 AS (
    ...
    FROM (SELECT dia, closer_r1, count(*) AS q FROM vd_base GROUP BY 1,2) x
    GROUP BY dia
  ),
  vd_closer_r2 AS (
    SELECT dia, jsonb_object_agg(closer_r2, q) AS obj
    FROM (SELECT dia, closer_r2, count(*) AS q FROM vd_base GROUP BY 1,2) x
    GROUP BY dia
  ),
  vd_sdr AS (
...
-- LAST TOUCH na atribuição de SDR da venda (agg=semana_resultado)
