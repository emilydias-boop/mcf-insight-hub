-- Fix: Corrigir timezone para contar agendamentos no dia correto (Brasília)
-- Agendamentos feitos após 21:00 BRT estavam sendo contados no dia seguinte
-- porque o PostgreSQL usava UTC para extrair a data
...
BEGIN
  WITH sdr_stats AS (
    SELECT 
      ...
      -- Agendamentos no período usando booked_at (data real do agendamento) no timezone de Brasília
      ...
      COUNT(CASE 
        ...
        THEN 1 
      END) as agendamentos,
      -- R1 Agendada: reuniões marcadas PARA o período (scheduled_at) no timezone de Brasília
      COUNT(CASE WHEN (ms.scheduled_at AT TIME ZONE 'America/Sao_Paulo')::date >= start_date::DATE 
                  AND (ms.scheduled_at AT TIME ZONE 'America/Sao_Paulo')::date <= end_date::DATE THEN 1 END) as r1_agendada,
      ...
      COUNT(CASE WHEN (ms.scheduled_at AT TIME ZONE 'America/Sao_Paulo')::date >= start_date::DATE 
                  ...
                  AND msa.status IN ('completed', 'contract_paid', 'refunded') THEN 1 END) as r1_realizada,
      -- No-show no período
      COUNT(CASE WHEN (ms.scheduled_at AT TIME ZONE 'America/Sao_Paulo')::date >= start_date::DATE 
                  AND (ms.scheduled_at AT TIME ZONE 'America/Sao_Paulo')::date <= end_date::DATE 
                  AND msa.status = 'no_show' THEN 1 END) as no_shows,
      -- Contrato pago no período: usar contract_paid_at (igual closers) com fallback para scheduled_at
      -- Inclui tanto contract_paid quanto refunded - aplicar timezone de Brasília
      COUNT(CASE 
        WHEN msa.status IN ('contract_paid', 'refunded')
         AND (
           -- Contratos COM contract_paid_at: usar data do pagamento no timezone de Brasília
           (msa.contract_paid_at IS NOT NULL 
            AND (msa.contract_paid_at AT TIME ZONE 'America/Sao_Paulo')::date >= start_date::DATE 
            AND (msa.contract_paid_at AT TIME ZONE 'America/Sao_Paulo')::date <= end_date::DATE)
           -- Fallback para contratos antigos sem timestamp: usar scheduled_at no timezone de Brasília
           OR (msa.contract_paid_at IS NULL 
               AND (ms.scheduled_at AT TIME ZONE 'America/Sao_Paulo')::date >= start_date::DATE 
               AND (ms.scheduled_at AT TIME ZONE 'America/Sao_Paulo')::date <= end_date::DATE)
         )
        THEN 1 
      END) as contratos
    ...
    WHERE msa.status != 'cancelled'
      ...
      AND (
        -- Include if booked_at is in range OR scheduled_at is in range OR contract_paid_at is in range
        -- Usar timezone de Brasília para todos os filtros de inclusão
        ((COALESCE(msa.booked_at, msa.created_at) AT TIME ZONE 'America/Sao_Paulo')::date >= start_date::DATE 
         AND (COALESCE(msa.booked_at, msa.created_at) AT TIME ZONE 'America/Sao_Paulo')::date <= end_date::DATE)
        OR ((ms.scheduled_at AT TIME ZONE 'America/Sao_Paulo')::date >= start_date::DATE 
            AND (ms.scheduled_at AT TIME ZONE 'America/Sao_Paulo')::date <= end_date::DATE)
        OR (msa.contract_paid_at IS NOT NULL 
            AND (msa.contract_paid_at AT TIME ZONE 'America/Sao_Paulo')::date >= start_date::DATE 
            AND (msa.contract_paid_at AT TIME ZONE 'America/Sao_Paulo')::date <= end_date::DATE)
      )
