-- Update function to count only original appointments and 1st reschedule after no-show
-- 2nd+ reschedules do NOT count as new appointments for the SDR
...
BEGIN
  WITH sdr_stats AS (
    SELECT 
      ...
      -- Conta apenas: originais (sem parent) OU 1º reagendamento (parent sem avô)
      COUNT(CASE 
        WHEN msa.created_at::date >= start_date::DATE 
         AND msa.created_at::date <= end_date::DATE
         AND (
           msa.parent_attendee_id IS NULL  -- Agendamento original
           OR parent_msa.parent_attendee_id IS NULL  -- 1º reagendamento (parent existe mas não tem avô)
         )
        THEN 1 
      END) as agendamentos,
