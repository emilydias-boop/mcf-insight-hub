CREATE OR REPLACE FUNCTION public.oi_venda_confirmada(p_deal_id uuid, p_email text, p_phone text, p_de date, p_ate date)
RETURNS boolean
LANGUAGE sql
STABLE
...
AS $fn$
  SELECT EXISTS (
    SELECT 1 FROM public.hubla_transactions ht
    WHERE public.oi_is_venda_produto(ht.product_name)
      ...
      AND (ht.sale_date AT TIME ZONE 'America/Sao_Paulo')::date BETWEEN p_de AND p_ate
      AND (
        ht.linked_deal_id = p_deal_id
        OR (coalesce(btrim(p_email),'') <> '' AND lower(btrim(ht.customer_email)) = lower(btrim(p_email)))
        OR (
          length(regexp_replace(coalesce(p_phone,''), '[^0-9]', '', 'g')) >= 9
          AND right(regexp_replace(coalesce(ht.customer_phone,''), '[^0-9]', '', 'g'), 9)
...
GRANT EXECUTE ON FUNCTION public.oi_venda_confirmada(uuid, text, text, date, date) TO authenticated, service_role;
...
BEGIN
  ...
  a010_vd AS (
    SELECT (ht.sale_date AT TIME ZONE 'America/Sao_Paulo')::date AS dia,
           count(*) AS qtd,
    ...
    WHERE coalesce(ht.product_name,'') ILIKE 'A010%'
      ...
      AND ht.sale_date IS NOT NULL
      AND (ht.sale_date AT TIME ZONE 'America/Sao_Paulo')::date BETWEEN p_from AND p_to
    GROUP BY 1
  ),
  ct AS (
  ...
  vd_base AS (
    SELECT (dl.stage_moved_at AT TIME ZONE 'America/Sao_Paulo')::date AS dia,
           dl.id AS deal_id,
           coalesce(dl.name,'Sem nome') AS deal_nome,
           coalesce(dl.value,0) AS valor,
           public.oi_classify_origem(dl.product_name, dl.tags, cc.tags) AS origem,
           public.oi_venda_confirmada(dl.id, cc.email, cc.phone, p_from, p_to) AS confirmada,
           coalesce((
             ...
             WHERE (ht.linked_deal_id = dl.id
                    ...
                    OR (length(regexp_replace(coalesce(cc.phone,''),'[^0-9]','','g')) >= 9
                        AND right(regexp_replace(coalesce(ht.customer_phone,''),'[^0-9]','','g'),9)
                            = right(regexp_replace(coalesce(cc.phone,''),'[^0-9]','','g'),9)))
           ...
           coalesce((
             ...
             WHERE a3.deal_id = dl.id
               ...
               AND coalesce(a3.booked_at, a3.created_at) <= dl.stage_moved_at
             ORDER BY coalesce(a3.booked_at, a3.created_at) DESC
             LIMIT 1
  ...
  vd AS (
    SELECT dia, count(*) AS qtd, coalesce(sum(valor),0) AS valor FROM vd_base WHERE confirmada GROUP BY 1
  ),
  pt AS (
    SELECT dia, count(*) AS qtd, jsonb_agg(deal_nome ORDER BY deal_nome) AS nomes
    FROM vd_base WHERE NOT confirmada GROUP BY 1
  ),
  vd_prod AS (
    SELECT dia, jsonb_object_agg(produto, jsonb_build_object('qtd', q, 'valor', v)) AS obj
    FROM (SELECT dia, produto, count(*) AS q, coalesce(sum(valor),0) AS v FROM vd_base WHERE confirmada GROUP BY 1,2) x
    GROUP BY dia
  ),
  vd_orig AS (
    SELECT dia, jsonb_object_agg(origem, jsonb_build_object('qtd', q, 'valor', v)) AS obj
    FROM (SELECT dia, origem, count(*) AS q, coalesce(sum(valor),0) AS v FROM vd_base WHERE confirmada GROUP BY 1,2) x
    GROUP BY dia
  ),
  vd_closer_r1 AS (
    SELECT dia, jsonb_object_agg(closer_r1, q) AS obj
    FROM (SELECT dia, closer_r1, count(*) AS q FROM vd_base WHERE confirmada GROUP BY 1,2) x
    GROUP BY dia
  ),
  vd_closer_r2 AS (
    SELECT dia, jsonb_object_agg(closer_r2, q) AS obj
    FROM (SELECT dia, closer_r2, count(*) AS q FROM vd_base WHERE confirmada GROUP BY 1,2) x
    GROUP BY dia
  ),
  vd_sdr AS (
    SELECT dia, jsonb_object_agg(sdr_venda, q) AS obj
    FROM (SELECT dia, sdr_venda, count(*) AS q FROM vd_base WHERE confirmada GROUP BY 1,2) x
    GROUP BY dia
  )
  SELECT jsonb_build_object('dias', coalesce(jsonb_agg(jsonb_build_object(
      ...
      'vendas_confirmadas', coalesce(vd.qtd,0),
      'vendas_valor', coalesce(vd.valor,0),
      'parceiros_tag', coalesce(pt.qtd,0),
      'parceiros_tag_nomes', coalesce(pt.nomes,'[]'::jsonb),
  ...
  LEFT JOIN a010_vd ON a010_vd.dia = dias.dia
  ...
  LEFT JOIN pt ON pt.dia = dias.dia
  LEFT JOIN vd_prod ON vd_prod.dia = dias.dia
  LEFT JOIN vd_orig ON vd_orig.dia = dias.dia
  LEFT JOIN vd_closer_r1 ON vd_closer_r1.dia = dias.dia
  LEFT JOIN vd_closer_r2 ON vd_closer_r2.dia = dias.dia
...
BEGIN
  ...
  vd_base AS (
    SELECT dl.id AS deal_id,
           ...
           coalesce(dl.value,0) AS valor,
           public.oi_classify_origem(dl.product_name, dl.tags, cc.tags) AS origem,
           public.oi_venda_confirmada(dl.id, cc.email, cc.phone, v_vd_de, v_vd_ate) AS confirmada,
           coalesce((
             SELECT ht.product_name
             FROM hubla_transactions ht
             WHERE (ht.linked_deal_id = dl.id
  ...
  vd AS (SELECT count(*) AS qtd, coalesce(sum(valor),0) AS valor FROM vd_base WHERE confirmada),
  pt AS (
    SELECT count(*) AS qtd, coalesce(jsonb_agg(deal_nome ORDER BY deal_nome),'[]'::jsonb) AS nomes
    FROM vd_base WHERE NOT confirmada
  ),
  vd_prod AS (
    SELECT jsonb_object_agg(produto, jsonb_build_object('qtd', q, 'valor', v)) AS obj
    FROM (SELECT produto, count(*) AS q, coalesce(sum(valor),0) AS v FROM vd_base WHERE confirmada GROUP BY 1) x
  ),
  vd_orig AS (
    SELECT jsonb_object_agg(origem, jsonb_build_object('qtd', q, 'valor', v)) AS obj
    FROM (SELECT origem, count(*) AS q, coalesce(sum(valor),0) AS v FROM vd_base WHERE confirmada GROUP BY 1) x
  ),
  vd_closer_r1 AS (
    SELECT jsonb_object_agg(closer_r1, q) AS obj
    FROM (SELECT closer_r1, count(*) AS q FROM vd_base WHERE confirmada GROUP BY 1) x
  ),
  vd_closer_r2 AS (
    SELECT jsonb_object_agg(closer_r2, q) AS obj
    FROM (SELECT closer_r2, count(*) AS q FROM vd_base WHERE confirmada GROUP BY 1) x
  ),
  vd_sdr AS (
    SELECT jsonb_object_agg(sdr_venda, q) AS obj
    FROM (SELECT sdr_venda, count(*) AS q FROM vd_base WHERE confirmada GROUP BY 1) x
  )
  SELECT jsonb_build_object(
    ...
    'vendas', jsonb_build_object(
      ...
      'parceiros_tag', coalesce(pt.qtd,0),
      'parceiros_tag_nomes', coalesce(pt.nomes,'[]'::jsonb),
      'conversao_caucao_venda', CASE WHEN coalesce(cauc.total,0) > 0
        THEN round((coalesce(vd.qtd,0)::numeric / cauc.total::numeric) * 100, 1) ELSE 0 END,
      'vendas_por_produto', coalesce(vd_prod.obj,'{}'::jsonb),
      'vendas_por_origem', coalesce(vd_orig.obj,'{}'::jsonb),
      'vendas_por_closer_r1', coalesce(vd_closer_r1.obj,'{}'::jsonb),
      'vendas_por_closer_r2', coalesce(vd_closer_r2.obj,'{}'::jsonb),
  ...
  FROM lc, a010_vd, ct, ag, r1_marc, r1_ok, r1_ns, r1_ns_sdr, cauc, r2, r2_closer,
       vd, pt, vd_prod, vd_orig, vd_closer_r1, vd_closer_r2, vd_sdr;
