...
BEGIN
  ...
  -- Parents (sem sufixo -offer-) cadastrados em product_configurations Incorporador.
  -- Usado para que bumps "herdem" o target_bu do produto principal.
  incorp_parents AS (
    SELECT DISTINCT ht_parent.hubla_id AS parent_id
    FROM hubla_transactions ht_parent
    INNER JOIN product_configurations pc_parent
      ON LOWER(ht_parent.product_name) = LOWER(pc_parent.product_name)
      AND pc_parent.target_bu = 'incorporador'
      AND pc_parent.is_active = true
    WHERE ht_parent.hubla_id NOT LIKE '%-offer-%'
  ),
  ranked_transactions AS (
    ...
    WHERE
      ...
      AND (
        -- Caso normal: produto cadastrado em Incorporador
        pc.id IS NOT NULL
        -- Caso bump: herda do parent se este estiver cadastrado em Incorporador
        OR (
          ht.hubla_id LIKE '%-offer-%'
          AND SPLIT_PART(ht.hubla_id, '-offer-', 1) IN (SELECT parent_id FROM incorp_parents)
        )
      )
  )
  ...
  LEFT JOIN product_configurations pc2
  ...
  WHERE
    ...
    AND ht2.source IN ('hubla', 'manual', 'make', 'mcfpay', 'kiwify')
    ...
    AND (
      pc2.id IS NOT NULL
      OR (
        ht2.hubla_id LIKE '%-offer-%'
        AND SPLIT_PART(ht2.hubla_id, '-offer-', 1) IN (
          SELECT DISTINCT ht_parent.hubla_id
          FROM hubla_transactions ht_parent
          INNER JOIN product_configurations pc_parent
            ON LOWER(ht_parent.product_name) = LOWER(pc_parent.product_name)
            AND pc_parent.target_bu = 'incorporador'
