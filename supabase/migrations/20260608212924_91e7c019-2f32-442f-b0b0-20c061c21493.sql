...
BEGIN
  ...
  IF v_employee IS NULL THEN RAISE EXCEPTION 'Colaborador não encontrado'; END IF;
  SELECT * INTO v_cargo FROM public.cargos_catalogo WHERE id = p_cargo_catalogo_id;
  IF v_cargo IS NULL THEN RAISE EXCEPTION 'Cargo não encontrado'; END IF;
  IF p_valid_from IS NULL THEN RAISE EXCEPTION 'Data inicial obrigatória'; END IF;
  ...
  VALUES (p_employee_id, p_cargo_catalogo_id, p_valid_from, NULL,
          COALESCE(p_motivo, 'Alteração retroativa de cargo'), v_caller);
  ...
  IF p_update_comp_plans THEN
    ...
    IF v_sdr_id IS NOT NULL THEN
      UPDATE public.sdr
      SET nivel = COALESCE(v_cargo.nivel, nivel), updated_at = now()
      WHERE id = v_sdr_id;

      UPDATE public.sdr_comp_plan
      SET vigencia_fim = p_valid_from - INTERVAL '1 day', updated_at = now()
      ...
      WHILE v_cursor_month <= v_end_month LOOP
        ...
        ELSE
          INSERT INTO public.sdr_comp_plan (
            sdr_id, cargo_catalogo_id, vigencia_inicio, vigencia_fim,
            ote_total, fixo_valor, variavel_total,
            meta_reunioes_agendadas, meta_reunioes_realizadas, status, criado_por
          ) VALUES (
            v_sdr_id, p_cargo_catalogo_id, v_month_first,
  ...
  VALUES (v_caller, 'cargo_change_retroativo', 'employees', p_employee_id,
          jsonb_build_object(
