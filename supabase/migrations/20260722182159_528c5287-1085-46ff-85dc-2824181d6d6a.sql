...
BEGIN
  ...
  IF v_code IN ('A001','A002','A003','A004','A009') THEN
    ...
    IF v_valor_total_hubla <= 0 THEN
      v_valor_total_hubla := public.ar_get_reference_price(v_code);
    END IF;
    v_tipo := CASE WHEN coalesce(NEW.total_installments,1) > 1 THEN 'parcelado' ELSE 'integral' END;
    v_status := CASE WHEN v_code = 'A009' THEN 'quitado' ELSE 'aberto' END;
  ELSE
    v_valor_total_hubla := coalesce(NEW.product_price, public.ar_get_reference_price(v_code));
    v_tipo := CASE WHEN coalesce(NEW.total_installments,1) > 1 THEN 'parcelado' ELSE 'integral' END;
    v_status := 'aberto';
  END IF;
  ...
  IF coalesce(NEW.total_installments,1) > 1 AND v_status <> 'quitado' THEN
    v_valor_pendente := greatest(v_valor_total_hubla - coalesce(NEW.product_price,0), 0);
    v_parcelas_extras := coalesce(NEW.total_installments,1) - 1;
