-- Corrige a função para excluir transações "parent" (somatório de ofertas)
-- Usa NOT IN para evitar erro de ambiguidade
...
BEGIN
  ...
  -- Extrai o UUID base de transações com sufixo -offer-N
  parent_ids AS (
    SELECT DISTINCT 
  ...
  -- CTE 1: Buscar transações excluindo newsale-% E parents
  all_transactions AS (
    SELECT 
      ht.id,
      ht.hubla_id,
