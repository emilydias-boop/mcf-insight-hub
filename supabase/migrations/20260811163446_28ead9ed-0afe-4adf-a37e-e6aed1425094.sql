ALTER TABLE public.meeting_slot_attendees ADD COLUMN IF NOT EXISTS refunded_at timestamptz NULL;

CREATE INDEX IF NOT EXISTS idx_msa_refunded_at ON public.meeting_slot_attendees (refunded_at) WHERE refunded_at IS NOT NULL;

CREATE OR REPLACE FUNCTION public.caucoes_efetivas(p_from date, p_to date, p_bu text DEFAULT 'incorporador'::text)
 RETURNS TABLE(attendee_id uuid, deal_id uuid, lead_name text, eff_date date, fonte text, contract_paid_at timestamp with time zone, closer_id uuid, closer_name text, closer_bu text, sdr_id uuid, sdr_email text, sdr_name text, segment text, valor numeric, origin_id uuid)
 LANGUAGE sql
...
ctx AS MATERIALIZED (
  ...
  WHERE (
    ...
    AND lower(COALESCE(ht.source, '')) NOT LIKE '%asaas%'
    AND COALESCE(ht.event_type, '') NOT ILIKE '%payment_received%'
    AND COALESCE(ht.event_type, '') NOT ILIKE '%payment.received%'
),
tx AS MATERIALIZED (
