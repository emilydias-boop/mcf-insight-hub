-- Consulta de impedimentos + situação do anúncio ao Dash
...
BEGIN
  ...
  IF r.consortium_card_id IS NOT NULL AND s.card_existe THEN
...
-- Etapa 6 -> 5 : desfaz "parcela inicial paga"
...
BEGIN
  ...
  IF r.consortium_card_id IS NOT NULL AND s.card_existe THEN
    ...
    -- Antes a acao desistia em silencio quando faltava data_reserva. Agora a
    -- reserva e ancorada na criacao da cota. data_reserva, tipo_registro e
    -- data_contratacao NAO sao observados pelo trigger de webhook de saida.
    v_reserva := COALESCE(c.data_reserva, r.cota_aberta_at::date, c.created_at::date);
    UPDATE consortium_cards
       SET tipo_registro = 'reserva',
           data_contratacao = NULL,
           data_reserva = v_reserva
     WHERE id = r.consortium_card_id;
  END IF;
