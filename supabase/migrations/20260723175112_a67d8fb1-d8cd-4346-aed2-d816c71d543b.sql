...
-- Restringe ar_create_from_hubla à lista oficial de produtos (A001, A002, A003, A004, A009)
...
BEGIN
  ...
  -- Somente produtos oficialmente habilitados no Financeiro › A Receber
  IF v_code IS NULL OR NOT v_code = ANY(ARRAY['A001','A002','A003','A004','A009']) THEN
    RETURN NEW;
  END IF;
...
-- Remove títulos e dependências de produtos que não fazem parte da regra oficial
-- (A000 - Contrato, A005-A008): não devem aparecer no Financeiro › A Receber
WITH alvos AS (
  SELECT id FROM public.ar_titulos
  WHERE product_code IS NOT NULL
    AND product_code NOT IN ('A001','A002','A003','A004','A009')
)
DELETE FROM public.ar_reembolsos WHERE titulo_id IN (SELECT id FROM alvos);
...
DELETE FROM public.ar_historico WHERE titulo_id IN (SELECT id FROM alvos);
...
DELETE FROM public.ar_parcelas WHERE titulo_id IN (SELECT id FROM alvos);

DELETE FROM public.ar_titulos
WHERE product_code IS NOT NULL
  AND product_code NOT IN ('A001','A002','A003','A004','A009');
