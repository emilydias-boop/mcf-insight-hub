-- Recria a função para excluir corretamente transações "parent" que têm filhos "-offer-"
-- A lógica anterior usava "NOT LIKE 'parent-%'" que não funcionava porque o padrão real é UUID simples
...
BEGIN
  RETURN QUERY
  WITH 
  -- CTE 0: Identificar hubla_ids que são "parents" de ofertas
  -- Extrai o prefixo UUID de transações como "4bcfffaa-b5aa-48df-9692-4981c257900b-offer-1"
  parent_ids AS (
    SELECT DISTINCT 
      SUBSTRING(hubla_id FROM '^([a-f0-9-]+)-offer-') as parent_hubla_id
  ...
  -- Exclui: newsale-%, e transações que são "parents" de ofertas
  all_transactions AS (
    SELECT 
      ht.id,
      ht.hubla_id,
    ...
    WHERE ht.source = 'hubla'
      ...
      -- Excluir transações que são "parents" de ofertas
      AND NOT EXISTS (
        SELECT 1 FROM parent_ids pi 
        WHERE pi.parent_hubla_id = ht.hubla_id
      )
  ),
