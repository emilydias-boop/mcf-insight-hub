...
BEGIN
  ...
  -- R1 attendees dentro do intervalo (com lookback), usados para match por email/telefone
  r1_att AS (
    SELECT
      ...
      ms.scheduled_at,
      LOWER(TRIM(COALESCE(cc.email, msa.attendee_email))) AS email_norm,
  ...
  -- Direct via linked_attendee_id
  direct_link AS (
    SELECT t.id AS tx_id, r.sdr_id, r.closer_id, r.scheduled_at
    FROM txn t
    JOIN r1_att r ON r.attendee_id = t.linked_attendee_id
  ),
  -- Match por email (mais recente antes da venda)
  email_match AS (
    SELECT DISTINCT ON (t.id) t.id AS tx_id, r.sdr_id, r.closer_id, r.scheduled_at
    FROM txn t
    JOIN r1_att r ON r.email_norm = t.email_norm AND t.email_norm <> ''
    WHERE r.scheduled_at <= (t.d + interval '1 day')::timestamptz
