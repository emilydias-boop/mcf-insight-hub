CREATE OR REPLACE FUNCTION public.oi_norm_tags(p_tags text[])
RETURNS text[]
LANGUAGE plpgsql
IMMUTABLE
...
DECLARE
  ...
  out_tags text[] := '{}'::text[];
BEGIN
  IF p_tags IS NULL THEN RETURN out_tags; END IF;
  FOREACH t IN ARRAY p_tags LOOP
    v := NULL;
    IF t IS NULL THEN CONTINUE; END IF;
    IF left(btrim(t),1) = '{' THEN
      BEGIN
        v := nullif(btrim(coalesce((btrim(t)::jsonb)->>'name','')),'');
    ...
    ELSE
      ...
      IF v IS NULL THEN CONTINUE; END IF;
    END IF;
    out_tags := out_tags || upper(v);
  END LOOP;
  RETURN out_tags;
END;
$$;

CREATE OR REPLACE FUNCTION public.oi_classify_origem(p_product_name text, p_deal_tags text[], p_contact_tags text[])
RETURNS text
LANGUAGE plpgsql
IMMUTABLE
SET search_path TO 'public'
AS $$
DECLARE
  pn text := btrim(coalesce(p_product_name,''));
  tg text[] := public.oi_norm_tags(p_deal_tags) || public.oi_norm_tags(p_contact_tags);
BEGIN
  IF pn ILIKE 'A010%' OR EXISTS (SELECT 1 FROM unnest(tg) x WHERE x LIKE 'A010%') THEN
    RETURN 'A010';
  END IF;
  IF pn ILIKE '%anamnese%' OR EXISTS (SELECT 1 FROM unnest(tg) x WHERE x LIKE '%ANAMNESE%') THEN
    RETURN 'Anamnese';
  END IF;
  IF pn ILIKE '%guia%' OR EXISTS (SELECT 1 FROM unnest(tg) x WHERE x LIKE '%GUIA%') THEN
    RETURN 'Guia';
  END IF;
  IF EXISTS (SELECT 1 FROM unnest(tg) x WHERE x LIKE '%OUTSIDE%' OR x LIKE '%PARCEIRO%') THEN
    RETURN 'Outside';
  END IF;
  IF pn <> '' AND lower(pn) <> 'clientdata inside' THEN
    RETURN pn;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.oi_norm_tags(text[]) TO service_role;
GRANT EXECUTE ON FUNCTION public.oi_classify_origem(text, text[], text[]) TO service_role;
