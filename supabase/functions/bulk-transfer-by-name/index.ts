...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { names, origin_id, new_owner_email, new_owner_profile_id, new_owner_name, dry_run = false } = await req.json();
    ...
    // 1. Find orphan deals matching the names in the specified origin/pipeline
