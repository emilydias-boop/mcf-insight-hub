...
function extractPhone(questions?: Array<{ question: string; answer: string }>): string | null {
  ...
  for (const qa of questions) {
    ...
    if (
      questionLower.includes("telefone") ||
      questionLower.includes("phone") ||
      questionLower.includes("whatsapp") ||
      questionLower.includes("celular")
    ) {
      return qa.answer;
    }
  }
    ...
    switch (event) {
      case "invitee.created": {
        ...
        if (!closerId) {
          ...
          return new Response(JSON.stringify({ error: "No closer available" }), {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
        ...
        if (existingSlot) {
          ...
          if (meetingLink || videoConferenceLink) {
            ...
            await supabase.from("meeting_slots").update(updateData).eq("id", slotId);
          ...
          if (slotError) {
            ...
            return new Response(JSON.stringify({ error: "Failed to create meeting slot", details: slotError }), {
              status: 500,
              headers: { ...corsHeaders, "Content-Type": "application/json" },
            });
          }
        ...
        return new Response(JSON.stringify({ success: true, slotId, attendeeId: attendee?.id }), {
          status: 200,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      case "invitee.canceled": {
        ...
        if (attendee) {
          ...
          await supabase.from("meeting_slot_attendees").update({ status: "canceled" }).eq("id", attendee.id);
          ...
          if (activeCount === 0) {
            ...
            await supabase.from("meeting_slots").update({ status: "canceled" }).eq("id", attendee.meeting_slot_id);
        ...
        if (meeting) {
          await supabase.from("meeting_slots").update({ status: "canceled" }).eq("id", meeting.id);
      ...
      case "invitee.no_show": {
        ...
        if (attendee) {
          // Update attendee status
          await supabase.from("meeting_slot_attendees").update({ status: "no_show" }).eq("id", attendee.id);
          ...
          if (showedUp === 0) {
            ...
            await supabase.from("meeting_slots").update({ status: "no_show" }).eq("id", attendee.meeting_slot_id);
        ...
        if (meeting) {
          await supabase.from("meeting_slots").update({ status: "no_show" }).eq("id", meeting.id);
