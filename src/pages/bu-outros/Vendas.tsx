...
export default function OutrosVendas() {
  ...
  const { data: transactions = [], isLoading, refetch, isFetching } = useTransactionsByBU('outros', filters);
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `vendas-outros-${format(new Date(), 'yyyy-MM-dd')}.csv`;
    a.click();
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Vendas - Outros</h1>
            <p className="text-muted-foreground">Transações de produtos diversos</p>
          </div>
        </div>
                  ...
                  <TableRow>
                    <TableCell colSpan={7} className="h-32 text-center text-muted-foreground">
                      {transactions.length === 0 
                        ? 'Nenhuma transação encontrada. Configure produtos com target_bu = "outros" na página de Produtos.'
