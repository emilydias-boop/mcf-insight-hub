...
export default function ConsorcioVendas() {
  ...
  const { data: transactions = [], isLoading, refetch, isFetching } = useTransactionsByBU('consorcio', filters);
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `vendas-consorcio-${format(new Date(), 'yyyy-MM-dd')}.csv`;
    a.click();
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Vendas - Consórcio</h1>
            <p className="text-muted-foreground">Transações de produtos de consórcio</p>
          </div>
        </div>
                  ...
                  <TableRow>
                    <TableCell colSpan={7} className="h-32 text-center text-muted-foreground">
                      {transactions.length === 0 
                        ? 'Nenhuma transação encontrada. Configure produtos com target_bu = "consorcio" na página de Produtos.'
