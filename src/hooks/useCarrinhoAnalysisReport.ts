  ...
  return useQuery({
    ...
    queryFn: async (): Promise<CarrinhoAnalysisData> => {
      ...
      if (phoneSuffixesForLookup.length > 0) {
        // Use the RPC function to find contacts with deals by phone
        const uniqueSuffixes = [...new Set(phoneSuffixesForLookup)];
        // Query contacts that have phones matching these suffixes
        ...
        // For each tx without deal, check if phone-matched contact has deals
        for (const tx of uniqueContracts) {
          const email = (tx.customer_email || '').toLowerCase().trim();
          const currentContact = contactMap.get(email);
          const currentContactId = currentContact?.id;
          if (currentContactId && dealMap.has(currentContactId)) continue; // already has deal
          ...
          // Find a phone-matched contact that HAS a deal
          for (const pc of phoneContacts) {
            if (dealMap.has(pc.id)) {
              // Replace the contact in contactMap for this email
              contactMap.set(email, { id: pc.id, phone: pc.phone });
              break;
            }
          }

          // If still no deal but found phone contact (might have meetings), use it
          if (!contactMap.get(email) || !dealMap.has(contactMap.get(email)!.id)) {
            const currentId = contactMap.get(email)?.id;
            if (!currentId && phoneContacts.length > 0) {
              contactMap.set(email, { id: phoneContacts[0].id, phone: phoneContacts[0].phone });
            }
          }
        }

        // Re-fetch deals and meetings for any new contact IDs added
