...
export function SdrRanking({ topSdrs }: SdrRankingProps) {
  ...
  return (
    <div className="flex flex-col gap-0.5 h-full min-h-0">
      {topSdrs.slice(0, 4).map((sdr, index) => (
        <Card key={sdr.email} className="bg-gradient-to-br from-card to-muted/30 border border-primary/20 flex-1">
          <CardContent className="p-1 h-full flex items-center gap-1">
            <span className="text-base">{medals[index]}</span>
            <div className="flex-1 min-w-0 flex flex-col justify-center">
              <h3 className="font-bold text-[10px] text-foreground truncate leading-tight">{sdr.nome}</h3>
