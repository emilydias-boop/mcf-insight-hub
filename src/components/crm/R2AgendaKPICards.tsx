  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(
          "customer_name, customer_email, customer_phone, customer_document, product_code, product_name, sale_date"
        )
        .eq("sale_status", "paid")
...
export function R2AgendaKPICards({ meetings, rangeStart, rangeEnd }: Props) {
  ...
  const kpis = useMemo(() => {
    ...
    return { agendadas, realizadas, noShowPct, noShows, vendas };
  }, [meetings, sales]);
  ...
  const cards = [
    ...
    },
    {
      ...
      sub: `${sales.length} venda(s) no período`,
      icon: DollarSign,
