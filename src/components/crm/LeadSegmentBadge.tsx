 ...
 * BU Crédito Imobiliário usa rótulos próprios: 'ICP', 'Parcial', 'Fora do ICP'.
...
// As chaves são comparadas já em maiúsculas (ver `key` no componente).
const CONFIG: Record<string, { label: string; color: string; title: string }> = {
  ...
  // BU Crédito Imobiliário
  ICP: { label: 'ICP', color: 'bg-green-600 hover:bg-green-600', title: 'Dentro do ICP' },
  PARCIAL: { label: 'Parcial', color: 'bg-amber-500 hover:bg-amber-500', title: 'Parcialmente dentro do ICP' },
  'FORA DO ICP': { label: 'Fora do ICP', color: 'bg-zinc-500 hover:bg-zinc-500', title: 'Fora do ICP' },
};
